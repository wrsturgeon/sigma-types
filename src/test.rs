    ...
    match maybe_also {
        ...
        None => {
            if i == 0 {
                ...
                TestResult::error("Positive but failed")
            }
        }
    }
}
        ...
        Ok(ok) => {
            if actually_positive {
                ...
                return TestResult::error(format!(
                    "Not positive, but deserialization succeeded: {i}"
                ));
            }
        }
        Err(e) => {
    ...
    match maybe_also {
        ...
        None => {
            if i == 0 {
                ...
                TestResult::error("Negative but failed")
            }
        }
    }
}
    ...
    let rust: Negative = match serde_json::from_str(&json) {
        Ok(ok) => {
            if actually_negative {
                ...
                return TestResult::error(format!(
                    "Not negative, but deserialization succeeded: {i}"
                ));
            }
        }
        Err(e) => {
